...
Tests PersonRunner._emit_event() (core/supervisor/runner.py) and
ProcessSupervisor._poll_person_events() (core/supervisor/manager.py).
"""
...
@pytest.fixture
def runner(tmp_path: Path) -> PersonRunner:
    """Create a PersonRunner with tmp_path-based directories."""
    return PersonRunner(
        person_name="test",
...
@pytest.fixture
def events_dir(runner: PersonRunner) -> Path:
    """Return the expected events directory for the runner."""
    return runner.shared_dir.parent / "run" / "events" / runner.person_name
...
def supervisor(tmp_path: Path) -> MagicMock:
    ...
    sup._poll_person_events = (
        ProcessSupervisor._poll_person_events.__get__(sup, ProcessSupervisor)
    )
    return sup
...
class TestEmitEvent:
    """Tests for PersonRunner._emit_event()."""

    def test_creates_event_file(self, runner: PersonRunner, events_dir: Path) -> None:
        """_emit_event creates a .json file in the correct directory."""
        runner._emit_event("person.heartbeat", {"name": "test"})

        json_files = list(events_dir.glob("*.json"))
        assert len(json_files) == 1
        assert json_files[0].suffix == ".json"

    def test_event_file_content(self, runner: PersonRunner, events_dir: Path) -> None:
        """File contains correct JSON with event type and data."""
        data = {"name": "test", "result": {"summary": "ok"}}
        runner._emit_event("person.heartbeat", data)

        json_files = list(events_dir.glob("*.json"))
        assert len(json_files) == 1

        content = json.loads(json_files[0].read_text(encoding="utf-8"))
        assert content["event"] == "person.heartbeat"
        assert content["data"] == data

    def test_creates_directory_if_missing(
        self, runner: PersonRunner, events_dir: Path
    ) -> None:
        """Events dir is created automatically if it doesn't exist."""
        assert not events_dir.exists()

        runner._emit_event("person.cron", {"task": "cleanup"})

        assert events_dir.is_dir()
        assert len(list(events_dir.glob("*.json"))) == 1

    def test_atomic_write(self, runner: PersonRunner, events_dir: Path) -> None:
...
# ── TestPollPersonEvents ──────────────────────────────────────────


class TestPollPersonEvents:
    """Tests for ProcessSupervisor._poll_person_events()."""
    ...
    async def test_reads_and_broadcasts_events(
        ...
        event = {"event": "person.heartbeat", "data": {"name": "alice"}}
        (events_dir / "0001.json").write_text(
            json.dumps(event), encoding="utf-8"
        )

        await supervisor._poll_person_events()

        supervisor._broadcast_event.assert_awaited_once_with(
            "person.heartbeat", {"name": "alice"}
        )
    ...
    async def test_deletes_processed_files(
        ...
        event_file.write_text(
            json.dumps({"event": "person.cron", "data": {"task": "report"}}),
            encoding="utf-8",
        )
    ...
    @pytest.mark.asyncio
    async def test_processes_multiple_persons(
        self, supervisor: MagicMock
    ) -> None:
        """Events from different person directories are all processed."""
        events_base = supervisor.run_dir / "events"

        # Create events for two different persons
        for person, event_type in [("alice", "person.heartbeat"), ("bob", "person.cron")]:
            person_dir = events_base / person
            person_dir.mkdir(parents=True)
            event = {"event": event_type, "data": {"name": person}}
            (person_dir / "0001.json").write_text(
                json.dumps(event), encoding="utf-8"
            )
        ...
        assert "person.heartbeat" in event_types
        assert "person.cron" in event_types
