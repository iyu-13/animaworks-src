...
| **C: 関連知識** | knowledge/ | 1200トークン | 密ベクトル類似度検索（RAG） | 拡散活性化による連想 |
...
| **F: エピソード** | episodes/ | 500トークン | 密ベクトル類似度検索（RAG） | 過去の行動記録の意味的検索 |
...
1. **前処理**（ConsolidationEngine）: エピソード収集、解決済みイベント収集、アクティビティログ要約を `consolidation_instruction` プロンプトに注入
2. **Anima実行**: Animaがツールで knowledge/ 更新、procedures/ 作成・更新、矛盾検出・解決を実行
3. **後処理**: `ForgettingEngine.synaptic_downscaling()`（メタデータベースの低活性マーク）、`ConsolidationEngine._rebuild_rag_index()`

### 週次統合フロー

1. **Anima実行**: `run_consolidation("weekly")` で `weekly_consolidation_instruction` に従い統合作業
