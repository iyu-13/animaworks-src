...
│   │   ├── conversation.py    #   会話記憶（state/conversation.json、ローリング圧縮）
...
ローリングチャット履歴。蓄積量が `conversation_history_threshold`（デフォルト30%）を超えると、古いターンをLLM要約で圧縮し、直近ターンは原文保持。`{anima_dir}/state/conversation.json` に保存。

### 4.9 短期記憶（ShortTermMemory）

セッション継続のための外部化記憶。Aモードでコンテキスト閾値を超えた際、`session_state.json`（機械用）と `session_state.md`（次回プロンプト注入用）を生成。`shortterm/{chat|heartbeat}/` でチャットとハートビートを分離し、`archive/` に自動退避。
...
- **イベントタイプ**: `message_received`, `message_sent`, `response_sent`, `channel_read`, `channel_post`, `human_notify`, `tool_use`, `heartbeat_start`, `heartbeat_end`, `cron_executed`, `memory_write`, `error`（`dm_sent` は `message_sent` の後方互換エイリアス、`dm_received` は `message_received` のエイリアス）
...
- **ファイル配置**: `{anima_dir}/shortterm/streaming_journal_{session_type}.jsonl`（`chat` / `heartbeat` で分離）
...
6チャネル並列の自動記憶検索をシステムプロンプトに注入する。`PrimingEngine` がプロンプト構築における唯一のアクティビティリーダー（海馬モデル）として機能する。

| チャネル | バジェット | ソース |
|---------|-----------|--------|
| A: sender_profile | 500トークン | shared/users/{sender}/index.md 直接読み取り |
| B: recent_activity | 1300トークン | ActivityLoggerから統一タイムライン取得 |
| C: related_knowledge | 700〜1200トークン | dense vector検索（RAG） |
| D: skill_match | 200トークン | description-based 3段階マッチング（スキル名のみ返却、最大5件） |
| E: pending_tasks | 300トークン | TaskQueueManager（永続タスクキュー要約） |
| F: episodes | 400〜500トークン | dense vector検索（RAG、episodes/） |

チャネル D はマッチしたスキル/手続きの**名前のみ**を返却する。全文注入は行わない（段階開示: 詳細はエージェントが必要時に `skill` ツールで読み込む）。
...
- **会話記憶** — state/conversation.json、ローリング圧縮。閾値超過時にLLM要約で古いターンを圧縮
- **短期記憶** — shortterm/chat/・heartbeat/ 分離。コンテキスト閾値超過時にJSON+MDで外部化
- **統一アクティビティログ** — activity_log/ に全インタラクションをJSONL時系列で記録。Priming統合で文脈継続
- **ストリーミングジャーナル（WAL）** — streaming_journal_{session_type}.jsonl。クラッシュ耐性のインクリメンタル永続化
...
- **Anima間メッセージ** — Messenger経由のテキスト通信。階層委任（supervisor → 配下、delegate_task ツール）
- **スーパーバイザーツール** — 部下を持つAnimaに自動有効化: disable/enable_subordinate, delegate_task, org_dashboard, audit_subordinate 等
...
- **外部ツール** — web_search, x_search, slack, chatwork, gmail, github, transcribe, aws_collector, local_llm, image_gen, google_calendar, call_human
...
- **永続タスクキュー** — task_queue.jsonl。滞留検知と委任プロンプト注入
