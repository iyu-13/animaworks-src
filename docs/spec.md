...
- 複数Personが **階層構造** で協調動作する（commander → worker 同期委任）
...
│   ├── person_factory.py      # Person生成（テンプレート/空白/MD）
│   ├── init.py                # ランタイム初期化
...
|`person_defaults`|全Personに適用されるデフォルト値             |
|`persons`       |Person単位のオーバーライド（未指定フィールドはdefaults適用）|

**PersonModelConfig フィールド:**
...
|`supervisor`                     |`str \| null`  |`null`                  |上位Personの名前                  |
...
- **Digital Personクラス** — カプセル化・自律動作。1 Person = 1ディレクトリ
...
- **Person間メッセージ** — Messenger経由のテキスト通信。階層委任（commander → worker 同期委任）
...
|config.md → config.json          |per-person MDから統合JSONへ。Pydanticバリデーション + per-person overrides|
|エージェントループは自作しない                |Claude Agent SDKに委譲。車輪の再発明はしない                             |
|実行モード3分岐                       |Claude SDK最優先、LiteLLM汎用、Assisted弱モデル対応。全てPersonカプセル内      |
