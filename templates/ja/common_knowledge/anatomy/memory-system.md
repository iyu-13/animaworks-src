...
あなたが会話を始めるたびに、Priming エンジンが複数チャネルで関連記憶を並列検索し、
システムプロンプトに自動注入する:

| チャネル | 何を検索するか | バジェット |
|---------|--------------|-----------|
| A: 送信者プロファイル | 相手のユーザー情報 | 500 |
| B: 直近の活動 | 最近の行動タイムライン | 1300 |
| C: 関連知識 | RAG でベクトル検索した知識 | 1200 |
| C0: 重要知識 | `[IMPORTANT]` タグ付きの知識（概要ポインタ） | 300 |
| D: スキルマッチ | メッセージに関連するスキル名 | 200 |
| E: 保留タスク | タスクキューの要約 + 完了タスク結果 | 500 |
| F: エピソード | RAG で検索した過去の体験 | 500 |

追加で以下も注入される:
- **直近の送信履歴**: 過去2時間のメッセージ送信・チャネル投稿（最大3件）
- **保留中の人間通知**: 直近24時間の `human_notify` イベント（500トークン）

**`[IMPORTANT]` タグと Channel C0**: `[IMPORTANT]` タグ付きの knowledge は、通常の RAG 検索（Channel C）に加えて、Channel C0 で概要ポインタとして常時注入される。概要のみが表示され、詳細は `read_memory_file` で確認できる。重要な業務ルールを knowledge/ に移す際は `[IMPORTANT]` を先頭に付けること。
...
| 段階 | 頻度 | 条件 | 処理 |
|------|------|------|------|
| Synaptic downscaling | 日次 | 90日間アクセスなし **かつ** 参照3回未満 | 活性レベルを `low` にマーク |
| Neurogenesis reorganization | 週次 | 活性レベルが `low` **かつ** 類似度 0.80 以上のペア | LLM でマージし、元チャンクを削除 |
| Complete forgetting | 月次 | 低活性90日超 **かつ** 参照2回以下 | アーカイブに移動・インデックス削除 |

**保護ルール**（忘却されない条件）:

| 対象 | 保護条件 |
|------|---------|
| `skills/`, `shared/users/` | 常時保護（忘却対象外） |
| `[IMPORTANT]` タグ付き | 最終アクセスから365日間は保護 |
| Knowledge: `success_count >= 2` | 保護 |
| Procedures: `importance == "important"` | 保護 |
| Procedures: `protected == True` | 保護 |
| Procedures: `version >= 3` | 保護 |

**Procedures の特別ルール**: 180日非活性かつ使用3回未満、または `failure_count >= 3` かつ `utility < 0.3` の場合はダウンスケール対象になる。
