...
| 書き込み元 | `plan_tasks`, `delegate_task`, SDK Task/Agent tool |
...
| 書き込み | `add_task`, `update_task`, `delegate_task` |
...
```
人間の指示 ─┬─► add_task ──────────────────► Layer 2 (task_queue.jsonl)
            └─► Anima が current_task.md に記録 ► Layer 3

plan_tasks ─┬─► state/pending/*.json ──────► Layer 1 (実行キュー)
...
|---------|---------|---------|---------|
| plan_tasks 投入 | JSON 作成 | pending で登録 | — |
...
1. **全てのタスクは Layer 2 に登録される**: plan_tasks, delegate_task, add_task いずれの経路でも task_queue.jsonl にエントリが存在する
