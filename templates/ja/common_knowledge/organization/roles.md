...
| `templates/{locale}/roles/{role}/permissions.md` | ロール別ツール許可 | ja / en |
...
| ロール | 概要 | デフォルトモデル | max_turns | max_chains | context_threshold |
|--------|------|-----------------|-----------|------------|-------------------|
| manager | 委任・報告・エスカレーション判断 | claude-opus-4-6 | 50 | 3 | 0.60 |
| engineer | コード実装・技術設計・テスト | claude-opus-4-6 | 200 | 10 | 0.80 |
...
1. **作成時** (`create_from_md`): `_apply_role_defaults()` が `permissions.md` と
   `specialty_prompt.md` を `animas/{name}/` にコピー。`_create_status_json()` が
   `defaults.json` の全フィールド（`background_model` 含む）を `status.json` にマージする。
2. **ロール変更時** (`animaworks anima set-role`): `permissions.md` と `specialty_prompt.md` を再コピー。
   `status.json` には `model`, `context_threshold`, `max_turns`, `max_chains`,
   `conversation_history_threshold` のみマージ（`background_model` は set-role では適用されない）。
