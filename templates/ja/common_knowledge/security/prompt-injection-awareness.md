...
（実装: `core/execution/_sanitize.py` の `TOOL_TRUST_LEVELS`、`core/memory/priming.py` の `format_priming_section`）

| trust | 意味 | 例 |
|-------|------|-----|
| `trusted` | 内部データ。安全に利用してよい | search_memory、read_memory_file、skill、add_task、update_task、list_tasks、post_channel、send_message、recent_outbound |
| `medium` | ファイル内容やコンテンツ検索。概ね信頼できるが注意が必要 | read_file、search_code、write_file、edit_file、execute_command、related_knowledge、episodes、sender_profile、pending_tasks |
| `untrusted` | 外部ソース。命令的テキストが含まれる可能性がある | web_search、web_fetch、read_channel、read_dm_history、slack_messages、chatwork_messages、gmail_read_body、x_search、related_knowledge_external |
