...
| リソース | 役割 |
|---------|------|
| `state/current_state.md` | 今取り組んでいるタスク（1つ） |
...
`current_state.md` は「今まさに取り組んでいるタスク」を記録するファイル。
...
タスクは以下の状態を遷移する。状態変更時は必ず current_state.md を更新すること（MUST）。
...
| 状態 | 意味 | current_state.md に記載 |
|------|------|----------------------|
| `received` | タスクを受け取ったが未着手 | pending.md に記載 |
| `in-progress` | 現在作業中 | current_state.md に記載 |
| `completed` | 完了 | idle に戻す + episodes/ にログ |
| `blocked` | ブロッカーにより中断 | current_state.md に blockers を記載 |
...
2. current_state.md に `status: in-progress` で記載する
3. episodes/ に「タスク着手」をログする（SHOULD）

**in-progress → completed（完了）**:
1. current_state.md を `status: idle` に戻す
2. episodes/ に「タスク完了」と結果の要約をログする（MUST）
3. タスクの依頼者に結果を報告する（assigned_by が他者の場合 MUST）
4. pending.md に次のタスクがあれば、最優先のものを current_state.md に移す

**in-progress → blocked（ブロック）**:
1. current_state.md の `status` を `blocked` に変更する
...
1. current_state.md の現在タスクの進捗をメモする（MUST）
2. 現在タスクを pending.md に戻す（状態と進捗メモ付き）
3. 新しいタスクを current_state.md に記載する
...
2. 優先度を再評価し、current_state.md に移すか判断する
