...
2. **MCP ツール (`mcp__aw__*`)**: AnimaWorks 固有の内部機能
   ...
   - タスク: `add_task`, `update_task`, `list_tasks`, `plan_tasks`
...
1. **内部ツール**: `send_message`, `search_memory`, `read_file`, `execute_command`, `add_task`, `plan_tasks` 等。ツール名をそのまま function calling で呼ぶ。`refresh_tools`, `share_tool` で個人・共通ツールを再スキャン可能
...
- **成果追跡**: report_procedure_outcome, report_knowledge_outcome
- **タスク系**: add_task, update_task, list_tasks, plan_tasks
...
| `list_tasks` | タスク一覧取得 |
| `plan_tasks` | 複数タスクをDAG投入し並列/直列実行 |

B-mode でも add_task, update_task, list_tasks, plan_tasks は利用可能。
