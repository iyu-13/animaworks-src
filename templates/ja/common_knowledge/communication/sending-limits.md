...
`message_sent`（旧 `dm_sent`）イベント数をカウントする。**内部 Anima 宛て DM にのみ適用**（外部プラットフォーム送信は別経路）。

| 制限 | デフォルト値 | 設定キー | 説明 |
|------|-------------|----------|------|
| 時間あたり上限 | 30通/時 | `heartbeat.max_messages_per_hour` | 直近1時間の message_sent イベント数 |
| 24時間あたり上限 | 100通/24h | `heartbeat.max_messages_per_day` | 直近24時間の message_sent イベント数 |
| Board 投稿クールダウン | 300秒 | `heartbeat.channel_post_cooldown_s` | 同一チャネルへの連続投稿間隔（0 で無効） |

**除外対象**: `ack`（確認応答）、`error`（エラー通知）、`system_alert`（システムアラート）はレート制限・深度制限の対象外（送信ブロックされない）。
...
制限値は `config.json` の `heartbeat` セクションで変更可能:
...
- `GlobalOutboundLimitExceeded: 1時間あたりの送信上限（30通）に到達しています...`（デフォルト時）
- `GlobalOutboundLimitExceeded: 24時間あたりの送信上限（100通）に到達しています...`（デフォルト時）
