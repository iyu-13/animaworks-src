...
直属部下でない相手への依頼は `send_message` で `intent="delegation"` を指定する。
...
ハートビートや cron に組み込むべき定期タスクを指示するパターン。継続タスクは `delegate_task` の対象外のため、`send_message` で `intent="delegation"` を指定する。
...
修正依頼は委譲にあたるため `intent="delegation"` を指定する。
...
以下のテンプレートを `send_message` で使用する際は、`intent` を適切に指定すること（依頼=delegation、報告=report、質問=question）。
