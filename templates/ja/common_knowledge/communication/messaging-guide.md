...
| `delegation` | タスク委譲（部下への指示） | delegate_task と併用する受領確認 |
...
| タスク委譲 | send_message (intent=delegation) | 部下への委譲受領確認 |
...
```bash
animaworks send {送信者名} {宛先} "メッセージ内容" [--intent report|delegation|question] [--reply-to ID] [--thread-id ID]
```
...
| `intent` | 送信者の意図 | `report`, `delegation`, `question` |
...
**症状**: `Error: DMのintentは 'report', 'delegation', 'question' のみ許可されています` と表示される

**原因**: `intent` を省略した、または acknowledgment・感謝・FYI を DM で送ろうとした

**対策**: DM では必ず `intent` に `report`, `delegation`, `question` のいずれかを指定する。了解・感謝・FYI は Board（post_channel）を使用する

### 宛先名の間違い

**症状**: 送信しても相手に届かない

**原因**: `to` パラメータの Anima 名が正確でない、または人間エイリアスが config.json に未登録

**対策**: Anima 名は大文字・小文字を区別する。人間への送信は `config.json` の `external_messaging.user_aliases` にエイリアスを登録すること。不明な場合は `search_memory(query="メンバー", scope="knowledge")` で組織メンバーを確認する
...
2. 時間制限に達した場合: 送信内容を current_task.md に記録し、次のセッションで送信する
...
| タスクの指示・確認 | 部下 | `send_message(to="worker", content="レポート作成をお願い", intent="delegation")` |
