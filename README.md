...
| コマンド | 説明 |
|---|---|
| `animaworks init [--force] [--reset]` | ランタイムディレクトリを初期化 |
| `animaworks create-person [--template NAME] [--from-md PATH] [--blank NAME]` | Person を新規作成 |
| `animaworks start` | サーバー起動 |
...
Person ごとにモデルと実行モードを選択できる。config.json の `persons.{name}.model` で指定。
...
- config.json で `role`（commander / worker）と `supervisor` を Person ごとに定義
- 委任（上→下）は同期、エスカレーション（下→上）は非同期（Messenger）
- 全 Person は DigitalPerson カプセル内で動作。独立デーモンは作らない
...
└── templates/           # デフォルト設定・プロンプトテンプレート
    ├── persons/{name}/  #   Person ディレクトリ（名前で識別）
    └── prompts/         #   再利用可能なプロンプト
```
